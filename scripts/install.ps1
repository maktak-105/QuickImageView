...
if ([string]::IsNullOrWhiteSpace($SourceExe)) {
    $defaultName = if ($Qt) { 'QuickImageViewQt.exe' } else { 'QuickImageView.exe' }
    $SourceExe = Join-Path $PSScriptRoot ('..\dist\binary\' + $defaultName)
}
$source = [IO.Path]::GetFullPath($SourceExe)
$target = [IO.Path]::GetFullPath($InstallDirectory)
$iconSource = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\resources\icons\QuickImageView.ico'))
