...
2026-08-24、依頼者による実機確認で本書の修正項目はすべて期待どおりに動作することを確認した。

- 「メニュー項目の選択がどんどん遅くなる」も解消。毎回 `Menu.popup()` でポップアップを
  生成し直す構成が原因だったと判断してよい。標準 `MenuBar` へ置換したことで
  Qtがメニューを永続管理するようになり、生成の繰り返しがなくなった。
