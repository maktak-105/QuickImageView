...
invariants:
- INV-1 原本ファイルは絶対に上書きしない: VERIFY `powershell -NoProfile -ExecutionPolicy Bypass -File tests/audit_current_state.ps1 -Invariant OriginalProtection` (原本同一パス/既存ファイル上書き拒否の実装根拠または検証欠落を見逃さず、違反・未判定をFAILにする)。
- INV-2 Explorer登録は存在確認だけで合格にしない: VERIFY `powershell -NoProfile -ExecutionPolicy Bypass -File tests/audit_current_state.ps1 -Invariant ExplorerLaunchEvidence` (実際のExplorer起動証拠がなければBLOCKED判定を要求し、PASSを許さない)。
...
acceptance_criteria:
- 監査成果物が、実装ファイル・テスト・配布スクリプト・Explorer登録・UI E2E・原本保護を列挙し、各項目をPASS/BLOCKED/FAILのいずれかで分類する。VERIFY `powershell -NoProfile -ExecutionPolicy Bypass -File tests/audit_current_state.ps1 -RequireCompleteMatrix` (必須分類・証拠パス・判定が欠けるとFAILする)。
- CMake構成とビルドが再現可能であることを確認し、失敗を隠さず記録する。VERIFY `cmake -S . -B build -G "MinGW Makefiles"; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; cmake --build build --clean-first` (構成またはビルド失敗で非ゼロ終了)。
- CTestの既存self-testを実行し、実終了コードと出力を証拠化する。VERIFY `ctest --test-dir build --output-on-failure` (テスト失敗で非ゼロ終了)。
- 監査成果物はUIの単体起動・ファイル選択・画像表示・ズーム/パン/右クリック、Explorer右クリック起動について、実機操作証拠がない限りBLOCKEDと判定する。VERIFY `powershell -NoProfile -ExecutionPolicy Bypass -File tests/audit_current_state.ps1 -RequireEvidenceFor UiE2E,ExplorerE2E` (証拠なしPASSまたはプロセス生存のみのPASSでFAILする)。
- 変更一覧が本要求のscope内に収まり、core/**・scripts/**・CMakeLists.txt・README.mdが変更されていないことを確認する。VERIFY `git diff --name-only -- core scripts CMakeLists.txt README.md` (対象変更が1件でもあればFAILする)。
expected_reply:
- changed_files
