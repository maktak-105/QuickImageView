...
Quickアプリ標準構成に合わせ、ソース、ビルドスクリプト、開発・配布文書を分離します。CMake中間生成物は`build/intermediate/`、完成した実行ファイルとQtランタイムは`dist/`直下に置きます。
...
├── src/app/                 C++エンジン・Qt連携・エントリーポイント・アイコン
...
`scripts/`にはアプリ全体のビルド（`build.bat`、`build_qt.py`）、Qtランタイム配置（`deploy_qt.py`）、インストール・アンインストール、ZIP作成処理を置きます。CMake中間生成物とZIPステージングは`build/intermediate/`へ、完成したバイナリは`dist/`直下へ出力します。Codexスキルは`.agents/skills/`、検査計画とループ状態は`docs/loop/`に置きます。
