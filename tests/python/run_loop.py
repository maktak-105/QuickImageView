...
def main() -> int:
    ...
    build = root / "build" / "intermediate" / "native"
    exe = root / "dist" / "QuickImageView.exe"
    readme = root / "README.md"
    ledger = root / "tests" / "python" / "operations.json"
    ...
    payload = {"schema_version": 1, "source": "README.md + README_jp.md -> tests/python/operations.json", "ui_first": True, "excluded_from_app_ui": sorted(EXCLUDED_FROM_APP_UI), "flow": ["build", "install", "ui_installed", "supplemental"], "ui_results": ui_results, "steps": steps, "supplemental": supplemental}
