...
void UpdateWindowTitle(HWND window) {
    std::wstring title = L"QuickImageView 3.1.3";
...
void UpdateAboutTexts() {
    ...
    SetWindowTextW(GetDlgItem(g_aboutWindow, kAboutVersion), L"Ver. 3.1.3");
...
LRESULT CALLBACK AboutWindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_CREATE) {
        ...
        CreateWindowW(L"STATIC", L"Ver. 3.1.3", WS_CHILD | WS_VISIBLE | SS_CENTER,
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR commandLine, int showCommand) {
    ...
    if (commandLine && wcscmp(commandLine, L"--help") == 0) {
        MessageBoxW(nullptr, Ui(L"QuickImageView 3.1.3\n画像ファイルを引数に指定してください。",
                                L"QuickImageView 3.1.3\nPass an image file as an argument."),
                    L"QuickImageView", MB_OK | MB_ICONINFORMATION);
        return 0;
    }
    ...
    HWND window = CreateWindowExW(extendedStyle, kClassName, L"QuickImageView 3.1.3",
    ...
    BuildMenu(window);
    if (filePath && *filePath && g_bitmap) SetWindowTextW(window, (L"QuickImageView 3.1.3 - " + g_fileName).c_str());
